...
export default function App() {
  ...
  const addRecipe = () => { const r=makeRecipe(); setRecipes(rs=>[...rs,r]); setEditId(r.id); };
          ...
          {recipes.map(r=>{
            ...
            return <Card key={r.id} style={{padding:0,overflow:"hidden"}}>
              ...
              <div style={{padding:"10px 18px",background:"#FFFFFF",borderTop:"0.5px solid #E0DED8",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <button onClick={()=>setRecipes(rs=>rs.filter(x=>x.id!==r.id))} style={{fontSize:12,color:"#9E3A3A",background:"none",fontWeight:600,flexShrink:0}}>Delete</button>
              </div>
            </Card>;
          })}
